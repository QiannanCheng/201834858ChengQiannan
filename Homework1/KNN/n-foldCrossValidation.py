...
kList=list(range(10,51,10)) #得到k的取值列表[10,20,30,40,50]
...
for k in kList:
    ...
    for i in range(5): #0,1,2,3,4
        ...
        #record
        recordf=open('record.txt','a')
        recordf.write('ErrorRate='+str(er)+'\n')
        recordf.close()
        ersum+=er 
    average=ersum/5
    ...
    #record
    recordf=open('record.txt','a')
    recordf.write('k='+str(k)+', AverageErrorRate='+str(average)+'\n')
    recordf.close()
...
recordf=open('record.txt','a')
recordf.write(str(kList))
recordf.write(str(averageList))
recordf.write('Optimal k: '+str(opk)+', AverageErrorRate: '+str(minErrorRate)+'\n')
recordf.close()
#将最优k值在测试数据上进行测试
...
#record
recordf.open('record.txt','a')
recordf.write('ErrorRate on testData: '+str(result))
recordf.close()
